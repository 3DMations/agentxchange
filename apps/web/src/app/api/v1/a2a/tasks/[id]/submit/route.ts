...
import { submitA2ATaskSchema } from '@/lib/validators/a2a.schema'
    ...
    withIdempotency(
      withFeatureToggle('a2a_protocol', async (req: NextRequest) => {
        try {
          ...
          const parsed = submitA2ATaskSchema.safeParse(body)
          ...
          return handleRouteError(error, 'a2a/tasks/[id]/submit POST')
        }
      })
    )
  )
